...
//       console.log(err)
    ...
    const model4 = await Live2DModel.from("/model/"+ ( "mao_pro_t03.model3.json"),{ autoUpdate: true })
    ...
    store.state.model4.scale.set(store.state.scale+0)
...
//adapt defferent parameterIds of models
   if (store.state.model4.internalModel.coreModel._parameterIds.includes("ParamMouthOpenY")){
        store.parameterIndex = store.state.model4.internalModel.coreModel.getParameterIndex("ParamMouthOpenY")
    }

    if (store.state.model4.internalModel.coreModel._parameterIds.includes("PARAM_MOUTH_OPEN_Y")){
        store.parameterIndex = store.state.model4.internalModel.coreModel.getParameterIndex("PARAM_MOUTH_OPEN_Y")
    }

    if (store.state.model4.internalModel.coreModel._parameterIds.includes("ParamMouthA")){
        store.parameterIndex = store.state.model4.internalModel.coreModel.getParameterIndex("ParamMouthA")
    }
